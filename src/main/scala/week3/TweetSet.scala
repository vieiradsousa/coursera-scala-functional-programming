...
abstract class TweetSet extends TweetSetInterface {
  ...
  def descendingByRetweet: TweetList = {
    if (isEmpty) Nil
      ...
      val mostRetweetedValue = mostRetweeted
      new Cons(mostRetweetedValue, remove(mostRetweetedValue).descendingByRetweet)
    }
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    def checkInclude(p: Tweet => Boolean, acc:TweetSet): TweetSet = {
  ...
  override def mostRetweeted: Tweet = {
    def compareTweets(tweetLeft: Tweet, tweetRight: Tweet): Tweet =
      if (tweetRight.retweets > tweetLeft.retweets) tweetRight else tweetLeft
