...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toSeq.groupBy(_.toChar).map{f => (f._1, f._2.length)}.toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.toMap.foldLeft(x.toMap)((xm, ym) => {
      val (c, n) = ym
      if (xm(c) - n > 0) xm + (c -> (xm(c) - n))
      else xm - c
    }).toList.sorted
...
object Dictionary {
  def loadDictionary: List[String] = {
    val wordstream = Option {
      getClass.getResourceAsStream(List("week6", "linuxwords.txt").mkString("/", "/", ""))
    } getOrElse {
